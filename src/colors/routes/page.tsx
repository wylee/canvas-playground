...
export default function Page() {
  ...
  // On mount, set canvas dimensions and start.
  useEffect(() => {
  ...
  useInterval(
    useCallback(() => setBgColor((bgColor + 1) % 360), [bgColor]),
    200,
  ...
  useInterval(
    useCallback(
      () =>
        setRadius(draw(canvasRef?.current as HTMLCanvasElement, radius, color)),
      [color, radius]
    ),
    50,
    canvasRef?.current && running,
